...
public class GameGUI extends JFrame implements ActionListener {
	...
	// String arrays to store categories and card details read in form the .txt
	// file
	private String[] categories;
	private String[] deck;

	// monospaced font in two sizes for use in all card display and message
	// fields
	...
	// labels to display number of cards in deck, number in user's hand, and
	// label to indicate
	// user's hand
	private JLabel userHandLabel, communalCount, humanCardCount;
	// the play button which the user will use to step through each round of the
	// game
	...
	/**
	 * Creates an instance of the Game GUI, based on how many opponents the user
	 * has selected
	 * 
	 * @param player
	 *            the number of opponents the user has selected for their game
	 *            in the StartGUI
	 **/
	public GameGUI(int player) {
		...
		GUIMiddle();
		GUIBottom();
		// int totalPlayers = player++;//adds user to number of opponents
		// selected
	...
	/**
	 * Method sets the game gui to show the correct number of opponents at the
	...
	/**
	 * Update Card count Method that updates the current card count on the GUI
	 * for each player
	 *
	 */
	...
	public void UpdatePlayer(int playerNumber) {
		...
		// case for when user won the previous round
		if (playerNumber == 0) {
	...
	public void ResetGUI() {
		...
		// (to prevent them clicking thru each and every play after they have
		// died)
		...
		// Doesn't set drop down box to disabled if it is players turn when GUI
		// is reset
	...
	/**
	 * method to handle user actions on GUI, in this case just user pressing
	 * 'play'
	 * 
	 * @author Lauren
	 **/
	...
	public String[] getCategories() {
		try {
			...
			System.err.println("file not found exception in GetCategories()");
		}
		return categories;
	}
	...
	public String[] getDeck() {
		try {
			...
			System.err.println("file not found exception in getDeck()");
		}
		return deck;
	}
	...
	public void GUIMessage() {
		...
		if (startGame.roundWasDraw()) {
			...
			roundresult = "\nYou won that round!" + "\nYou played the " + dinosaur + "'s " + characteristic
					+ ", which had a value of " + winValue;
		} else {
			roundresult = "\nRound was won by CPU " + winner + ".\nThey played the " + dinosaur + "'s "
